...
unordered_map<uint64, Player*> ChatServer::_players;
LockFreeObjectPoolTLS<Player, false> ChatServer::_player_pool;
list<Player*> ChatServer::_sectors[MAX_SECTOR_SIZE_Y][MAX_SECTOR_SIZE_X];
SRWLOCK ChatServer::_sector_lock[MAX_SECTOR_SIZE_Y][MAX_SECTOR_SIZE_X];
SRWLOCK ChatServer::_player_lock;

#pragma warning(disable: 26495)

ChatServer::ChatServer() :_player_num(0), _monitor_client(nullptr), _chat_solo(nullptr), _auth_solo(nullptr)
{
	for (int y = 0; y < MAX_SECTOR_SIZE_Y; y++)
	{
		for (int x = 0; x < MAX_SECTOR_SIZE_X; x++)
		{
			InitializeSRWLock(&_sector_lock[y][x]);
		}
	}

	InitializeSRWLock(&_player_lock);
...
void ChatServer::ChatInit(const WCHAR* text_file)
{
	TextParser parser;
	...
	_auth_solo = new AuthSolo(this, ContentLogicID::AuthSoloID);
	_chat_solo = new ChatSolo(this, ContentLogicID::ChatSoloID);

	//부착하기
	AttachLogicInstance(ContentLogicID::AuthSoloID, reinterpret_cast<LogicInstance*>(_auth_solo));
	AttachLogicInstance(ContentLogicID::ChatSoloID, reinterpret_cast<LogicInstance*>(_chat_solo));

}

void ChatServer::ChatInit(int32 max_player_num)
{
	ConsoleSize(43, 100);
...
bool ChatServer::OnAccept(uint64 session_id)
{
	_auth_solo->RegisterAccept(session_id);
...
bool ChatServer::OnConnectionRequest(WCHAR* IP, SHORT port)
{
	return true;
}

void ChatServer::OnError(const int8 log_level, uint64 session_id, int32 err_code, const WCHAR* cause)
{
...
void ChatServer::OnError(const int8 log_level, int32 err_code, const WCHAR* cause)
{
	WCHAR log_buff[df_LOG_BUFF_SIZE];
...
bool ChatServer::OnTimeOut(uint64 session_id)
{
...
void ChatServer::OnMonitor()
{
	...
	printf("----------------------------------------------------------------------------------------\n");
	printf("Auth Num: %d Player Num: %d  Req Login TPS: %d\n", _auth_num, player_num, req_login_tps);
	printf("PlayerChunk     [Cacpacity: %u Use: %u Release: %u]\n", _player_pool.GetChunkCapacityCount(), _player_pool.GetChunkNodeUseCount(), _player_pool.GetChunkReleaseCount());
	printf("PlayerChunkNode [Use: %u Release: %u ChunkSize: %d]\n", _player_pool.GetChunkUseCount(), _player_pool.GetChunkReleaseCount(), MAX_CHUNK_SIZE);
	...
	if (_monitor_client->GetConnectFlag())
	{
		...
		_monitor_client->SendMontiorPacket(en_PACKET_SS_MONITOR_DATA_UPDATE, dfMONITOR_DATA_TYPE_CHAT_UPDATE_TPS, 0, cur_time);
		...
		_monitor_client->SendMontiorPacket(en_PACKET_SS_MONITOR_DATA_UPDATE, dfMONITOR_DATA_TYPE_CHAT_UPDATEMSG_POOL, 0, cur_time);
