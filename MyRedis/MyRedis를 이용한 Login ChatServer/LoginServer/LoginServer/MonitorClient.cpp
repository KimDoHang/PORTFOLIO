...
{
	SendLoginPacket(en_PACKET_SS_MONITOR_LOGIN, ServerNo::LoginServerNo, 0);
}

void MonitorClient::OnRelease()
{
	_connect_flag = false;
	OnError(dfLOG_LEVEL_SYSTEM, 0, L"LoginServer LanClient Exit");
}
	...
	try
	{
		*packet >> type;
		...
		case en_PACKET_SS_MONITOR_LOGIN_RES:
			...
			break;
		default:
			OnError(dfLOG_LEVEL_SYSTEM, 0, L"LoginServer LanClient RecvMsg Type Error");
			Disconnect();
			break;
		}
...
void MonitorClient::OnError(const int8 log_level, int32 err_code, const WCHAR* cause)
{
	...
	{
		g_logutils->Log(LAN_LOG_DIR, L"Login_MonitorClient", cause);
	}
}
