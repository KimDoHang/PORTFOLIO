...
void {{ group_name }}::OnEnter(uint64 session_id, SessionInstance* instance)
{
}

void {{ group_name }}::OnLeave(uint64 session_id, SessionInstance* instance)
{
}

void {{ group_name }}::OnRelease(uint64 session_id, SessionInstance* instance)
{
}

void {{ group_name }}::OnFrame()
{
}

bool {{ group_name }}::OnRecvMsg(uint64 session_id, SessionInstance* instance, NetSerializeBuffer* msg)
{
...
void {{ group_name }}::Marshal_{{ function.name }}(uint64 session_id, {{ function.parameters | join(', ') }})
        {%- else %}
void {{ group_name }}::Marshal_{{ function.name }}(uint64 session_id)
        {%- endif %}
{
...
{%- for function in parser %}
     {%- if function.type == "CLIENT" %}
void {{ group_name }}::UnMarshal_{{ function.name }}(uint64 session_id, SessionInstance* instance, NetSerializeBuffer* msg)
{
		{%- for type, name, size, digit in function.update_params%}
		    {%- if type == "WCHAR" %}
				{%- if digit %}	
...
{%- endfor %}
    ...
    {%- for function in parser %}
        {%- if function.type == "CLIENT" %}
			{%- if function.update_params %}
void {{ group_name }}::Handle_{{ function.name }}(uint64 session_id, SessionInstance* instance, {{ function.parameters | join(', ') }})
			{%- else %}
void {{ group_name }}::Handle_{{ function.name }}(uint64 session_id, SessionInstance* instance)
			{%- endif %}
{
}
        {%- endif %}
